...
function generatePointAround(
  center, maxDistance = DEFAULT_DISTANCE_IN_KM, exprectedDistance
) {
  const randomRotation = math.random(0, 360)
  const randomRadius = exprectedDistance || math.random(0, maxDistance)
...
function PointInsideBBox(bbox, exprectedDistance) {
  const radius = geolib.getDistance(bbox.center, bbox.vertices[0])
  assert(
    _.isUndefined(exprectedDistance) || exprectedDistance <= radius
  ) // FIXME: silly check
  let result
  do {
    result = generatePointAround(bbox.center, radius, exprectedDistance)
